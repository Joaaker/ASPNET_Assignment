...
public class AuthController(IAuthService authService, IMemberService memberService, SignInManager<MemberEntity> signInManager, UserManager<MemberEntity> userManager, IHubContext<NotificationHub> notificationHub, INotificationService notificationService) : Controller
{
    ...
    {
        //Generated by ChatGPT 4o - checks for the remember me cookie and sets the email and remember me checkbox accordingly
        var model = new SignInViewModel();
        var rememberedEmail = Request.Cookies["rememberEmail"];
    ...
    public async Task<IActionResult> SignIn(SignInViewModel form, string returnUrl = "~/Projects/Index")
    {
        ...
        if (ModelState.IsValid)
        {
            ...
            if (result)
            {
                //Generated by ChatGPT 4o - functionality for the remember me checkbox, takes cookie consent into account
                if (form.RememberMe)
                {
                    var consentCookie = Request.Cookies["cookieConsent"];
                    if (!string.IsNullOrEmpty(consentCookie))
                    {
                        var consent = System.Text.Json.JsonSerializer.Deserialize<Domain.Models.CookieConsent>(consentCookie);
                        if (consent?.Functional == true)
                        {
                            ...
                            Response.Cookies.Append("rememberEmail", form.Email, cookieOptions);
                        }
                    }
                } else
